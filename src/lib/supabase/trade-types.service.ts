...
export async function getTradeTypes(params?: {
  ...
  try {
    ...
    let query = supabase.from("trade_types").select("*", { count: "exact" });
...
export async function getTradeType(
  identifier: string,
  by: "id" | "slug" = "id"
): Promise<{ tradeType: TradeType | null; error: string | null }> {
  try {
    ...
    return {
      tradeType: null,
      error: "An error occurred while fetching trade type",
    };
  }
}
 ...
 */
export async function createTradeType(
  input: CreateTradeTypeInput
): Promise<{ tradeType: TradeType | null; error: string | null }> {
  try {
    ...
    if (error || !tradeType) {
      return {
        tradeType: null,
        error: error?.message || "Failed to create trade type",
      };
    }
    ...
    return {
      tradeType: null,
      error: "An error occurred while creating trade type",
    };
  }
}
...
export async function updateTradeType(
  ...
  try {
    ...
    if (error || !tradeType) {
      return {
        tradeType: null,
        error: error?.message || "Failed to update trade type",
      };
    }
    ...
    return {
      tradeType: null,
      error: "An error occurred while updating trade type",
    };
  }
}
 ...
 * Delete trade type
 */
export async function deleteTradeType(
  id: string
  ...
  try {
    ...
    if (productsCount && productsCount > 0) {
      return {
        success: false,
        error: `Cannot delete trade type used by ${productsCount} products. Please remove associations first.`,
      };
    }

    const { error } = await supabase.from("trade_types").delete().eq("id", id);
    ...
    return {
      success: false,
      error: "An error occurred while deleting trade type",
    };
  }
}
